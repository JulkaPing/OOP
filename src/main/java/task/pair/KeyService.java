...
public class KeyService {
    ...
    public BigInteger getHash(Pair pair) {
        ...
        String generateKeyFirst = pair.getFirstKey();
        String generateKeySecond = pair.getSecondKey();

        String globalKey = generateKeyFirst.concat(generateKeySecond);

        StringBuilder stringBuilder = new StringBuilder();

        for (int i = 0; i < globalKey.length(); i++) {
            if (contains(globalKey.charAt(i))) {
                stringBuilder.append(globalKey.charAt(i));
            }
        }
